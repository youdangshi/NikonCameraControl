...
const GUIDES = {
  ...
  sta: {
    title: 'STA 局域网连接',
    intro: '相机和手机连接同一个路由器，App 使用相机在局域网中的 IP。',
    steps: ['手机连接家庭或工作 WiFi。', '相机无线菜单选择连接到路由器。', '在相机屏幕查看分配的 IP 地址。', '返回妮妮填写相机 IP，端口保持 15740。', '点击连接。'],
    tips: '手机与相机必须处于同一网段，路由器不能开启客户端隔离。',
  },
  usb: {
    title: 'USB Type-C 连接',
...
export default function MyCameraScreen() {
  ...
  useEffect(() => {
    const cleanups = [
      camera.on('status', data => {
        updateState({ connectionState: data.state, connectionMode: data.mode });
        if (data.state === 'session_open') {
          setError(null);
          const next = { mode: data.mode, time: Date.now(), host: data.host || '' };
          setLastConn(next);
  ...
  const changeMode = (value) => {
    ...
    setError(null);
    setShowGuide(false);
    if (value === 'wifi') setHost('192.168.1.1');
    else setHost('');
  };
  ...
  const connect = async () => {
    ...
    setLogs([]);
    try {
      const result = await camera.connectCamera(mode, host || '192.168.1.1', Number.parseInt(port, 10) || 15740);
    ...
    <div className="page">
      <div className="page-inner space-y-4">
        ...
        <section className="panel">
          ...
          <div className="px-4">
            <DetailRow label="连接方式" value={activeMode.label} />
            <DetailRow label="地址 / 端口" value={mode === 'usb' || mode === 'demo' ? activeMode.label : `${host || '未填写'}:${port}`} mono />
